...
static void IN_SDL_HandleSDLEvent(SDL_Event *event)
{
	...
	case SDL_EVENT_JOYSTICK_REMOVED:
		...
		{
			if (SDL_GetJoystickInstanceID(in_joysticks[i]) == event->jdevice.which)
				INL_StopJoy(i);
		}
		break;
	}
...
bool IN_SDL_StartJoy(int joystick)
{
	...
	SDL_JoystickGUID newGUID = SDL_GetJoystickInstanceGUID(joystick);
	for (int i = 0; i < IN_MAX_JOYSTICKS; ++i)
	{
		if (in_joystickPresent[i])
		{
			SDL_JoystickGUID GUID_i = SDL_GetJoystickGUID(in_joysticks[i]);
