...
static void VL_SDL3GPU_CreatePipeline()
{
	...
	colorDesc.format = SDL_GPU_TEXTUREFORMAT_R8G8B8A8_UNORM;//SDL_GPUGetSwapchainTextureFormat(vl_sdl3gpu_device, vl_sdl3gpu_window);
...
static void VL_SDL3GPU_CreateFramebuffers()
{
	...
	createInfo.format = SDL_GPU_TEXTUREFORMAT_R8G8B8A8_UNORM; //SDL_GPUGetSwapchainTextureFormat(vl_sdl3gpu_device, vl_sdl3gpu_window);
...
{
	SDL_SetWindowFullscreen(vl_sdl3gpu_window, vl_isFullScreen ? SDL_WINDOW_FULLSCREEN : 0);
	SDL_SetGPUSwapchainParameters(vl_sdl3gpu_device, vl_sdl3gpu_window, SDL_GPU_SWAPCHAINCOMPOSITION_SDR, (vl_swapInterval == 0) ? vl_sdl3gpu_supportedNoVsyncMode : SDL_GPU_PRESENTMODE_VSYNC);
...
static void VL_SDL3GPU_SetVideoMode(int mode)
{
	if (mode == 0xD)
	{
		...
		vl_sdl3gpu_window = SDL_CreateWindow(VL_WINDOW_TITLE,
			VL_DEFAULT_WINDOW_WIDTH(scale), VL_DEFAULT_WINDOW_HEIGHT(scale),
			SDL_WINDOW_RESIZABLE | (vl_isFullScreen ? SDL_WINDOW_FULLSCREEN : 0));

		SDL_SetWindowMinimumSize(vl_sdl3gpu_window, VL_VGA_GFX_SCALED_WIDTH_PLUS_BORDER / VL_VGA_GFX_WIDTH_SCALEFACTOR, VL_VGA_GFX_SCALED_HEIGHT_PLUS_BORDER / VL_VGA_GFX_HEIGHT_SCALEFACTOR);

		VL_SDL2GL_SetIcon(vl_sdl3gpu_window);
		...
		SDL_ClaimWindowForGPUDevice(vl_sdl3gpu_device, vl_sdl3gpu_window);

		// Prefer MAILBOX to VSYNC if we can.
		vl_sdl3gpu_supportedNoVsyncMode = SDL_GPU_PRESENTMODE_IMMEDIATE;
		if (SDL_WindowSupportsGPUPresentMode(vl_sdl3gpu_device, vl_sdl3gpu_window, SDL_GPU_PRESENTMODE_MAILBOX))
	...
	else
	{
		...
		SDL_DestroyWindow(vl_sdl3gpu_window);
	}
}
...
static void *VL_SDL3GPU_CreateSurface(int w, int h, VL_SurfaceUsage usage)
{
	...
	if (usage == VL_SurfaceUsage_FrontBuffer)
	{
		...
		SDL_GPUTransferBufferCreateInfo transferCreateInfo = { 0 };
...
static void VL_SDL3GPU_Present(void *surface, int scrlX, int scrlY, bool singleBuffered)
{
	...
	swapchainTexture = SDL_AcquireGPUSwapchainTexture(renderBuffer, vl_sdl3gpu_window, &newW, &newH);
	if (!swapchainTexture)
	{
	...
	VL_SDL3GPU_BindTexture(surface, innerPass);
	...
	SDL_GPUBlitRegion sourceTexRgn = {};
	sourceTexRgn.texture = vl_sdl3gpu_integerFramebuffer;
	sourceTexRgn.x = 0;
	sourceTexRgn.y = 0;
	sourceTexRgn.w = vl_integerWidth;
	sourceTexRgn.h = vl_integerHeight;

	SDL_GPUBlitRegion destTexRgn = {};
	destTexRgn.texture = swapchainTexture;
	destTexRgn.x = vl_fullRgn_x;
	destTexRgn.y = vl_fullRgn_y;
	destTexRgn.w = vl_fullRgn_w;
	destTexRgn.h = vl_fullRgn_h;

	SDL_GPUBlitInfo blitInfo = {};
	blitInfo.source = sourceTexRgn;
	blitInfo.destination = destTexRgn;
