...
static SDL_Window *vl_sdl3_window;
...
static void VL_SDL3_ResizeWindow()
{
	...
	if (vl_sdl3_scaledTarget)
	{
		...
		SDL_QueryTexture(vl_sdl3_scaledTarget, 0, 0, &curW, &curH);
...
static void VL_SDL3_SetVideoMode(int mode)
{
	if (mode == 0x0D)
	{
		...
		//VL_SDL3GL_SetIcon(vl_sdl3_window);
...
#endif
		vl_sdl3_renderer = SDL_CreateRenderer(vl_sdl3_window, NULL, flags);

		SDL_RendererInfo info;
		SDL_GetRendererInfo(vl_sdl3_renderer, &info);
...
static void VL_SDL3_Present(void *surface, int scrlX, int scrlY, bool singleBuffered)
{
	...
	SDL_FRect renderRect = {vl_renderRgn_x, vl_renderRgn_y, vl_renderRgn_w, vl_renderRgn_h};
	SDL_Rect fullRect = {(Sint16)vl_fullRgn_x, (Sint16)vl_fullRgn_y, vl_fullRgn_w, vl_fullRgn_h};
	SDL_FRect fullRectF = {(float)vl_fullRgn_x, (float)vl_fullRgn_y, vl_fullRgn_w, vl_fullRgn_h};
