...
// Sparky: user1 = charge countdown, user2 = shots remaining, user3 = version (0 == Sparky, 1 == Sparky Mk2), user4 = "flash" frames remaining
...
void CK5_SpawnSparky2(int tileX, int tileY)
{

	CK_object *new_object = CK_GetNewObj(false);
	new_object->type = CT5_SparkyII;
	new_object->active = OBJ_ACTIVE;
	new_object->zLayer = 0;
	new_object->posX = RF_TileToUnit(tileX);
	...
	new_object->user2 = 1;
	new_object->user3 = 1;
	CK_SetAction(new_object, CK_GetActionByName("CK5_ACT_Sparky2_0"));
}
...
void CK5_SparkyWait(CK_object *obj)
{
	...
	{
		if (obj->user3)
			obj->currentAction = CK_GetActionByName("CK5_ACT_Sparky2Search0");
		else
...
void CK5_SparkyPrepareCharge(CK_object *obj)
{
	...
	{
		if (obj->user3)
			obj->currentAction = CK_GetActionByName("CK5_ACT_Sparky2Charge0");
		else
...
void CK5_SparkySearchLeft(CK_object *obj)
{
	...
	if (delY < 0x200 && ck_keenObj->posX < obj->posX)
	{
		...
		if (obj->user3)
		{
			SD_PlaySound(CK_SOUNDNUM(SND_SparkyIICharge));
			obj->currentAction = CK_GetActionByName("CK5_ACT_Sparky2PrepCharge0");
		}
		else
		{
...
void CK5_SparkyCol(CK_object *obj1, CK_object *obj2)
{
	...
	else if (obj2->type == CT_Stunner)
	{
		...
		{
			if (obj1->user3)
				CK_StunCreature(obj1, obj2, CK_GetActionByName("CK5_ACT_Sparky2Stunned0"));
			else
		...
		else
		{
			...
			obj1->user4 = CK_INT(OSI_SparkyIIShotWhiteFrames, 3);
			obj1->user2--;
			CK_ShotHit(obj2);
			/* Charge in the direction of Keen. */
			if (ck_keenObj->posX < obj1->posX)
				obj1->xDirection = IN_motion_Left;
			else if (ck_keenObj->posX > obj1->posX)
				obj1->xDirection = IN_motion_Right;
			/* If our posX are identical, just charge in whatever direction we're currently facing. */
			SD_PlaySound(CK_SOUNDNUM(SND_SparkyIICharge));
			obj1->user1 = 3;
			obj1->currentAction = CK_GetActionByName("CK5_ACT_Sparky2PrepCharge0");
		}
	}
}
...
void CK5_SparkyTileCol(CK_object *obj)
{
	if (obj->xDirection == IN_motion_Right && obj->leftTI)
	{
		...
		if (obj->user3)
			CK_SetAction2(obj, CK_GetActionByName("CK5_ACT_Sparky2Turn0"));
		else
